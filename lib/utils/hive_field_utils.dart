...
class HiveFieldUtils {
  ...
  static SettingsProvider get _provider {
  ...
  static bool getFirstOpen() => HiveBoxes.firstOpenBox.get(firstOpen) ?? false;
  ...
  /// 获取字体缩放设置
  static double getFontScale() => _box.get(settingFontScale) as double? ?? 1.0;

  /// 获取新图标是否开启
  static bool getEnabledNewAppsIcon() =>
      _box.get(settingNewIcons) as bool? ?? true;

  /// 获取默认启动页index
  static int getHomeSplashIndex() =>
      _box.get(settingHomeSplashIndex) as int? ?? 0;

  /// 获取是否隐藏被屏蔽的动态
  static bool getEnabledHideShieldPost() =>
      _box.get(settingHideShieldPost) as bool? ?? true;

  /// 获取是否通过系统浏览器打开网页
  static bool getLaunchFromSystemBrowser() =>
      _box.get(settingLaunchFromSystemBrowser) as bool? ?? false;
  ...
  static Future<void> setLaunchFromSystemBrowser(bool enable) {
    _provider.launchFromSystemBrowser = enable;
